...
class Block(models.Model):
    ...
    def __hash__(self):
        return sha256(
            u"{}{}{}{}".format(
                self.index, self.data, self.previous_hash, self.nonce
            ).encode("utf-8")
        ).hexdigest()
    ...
    def is_valid_block(self, previous_block):
        if self.index != previous_block.index + 1:
            log.warning(
                "%s: Invalid index: %s and %s"
                % (self.index, self.index, previous_block.index)
            )
            return False
        if self.previous_hash != previous_block.hash:
            log.warning(
                "%s: Invalid previous hash: %s and %s"
                % (self.index, self.hash, previous_block.hash)
            )
            return False

        if self.__hash__() != self.hash and self.index > 1:
            log.warning(
                "%s: Invalid hash of content: %s and %s"
                % (self.index, self.hash, self.__hash__())
            )
            return False
...
class Chain(models.Model):
    ...
    def create_seed(self):
        ...
        seed = Block.generate_next(
            Block(hash=sha256("seed".encode("utf-8")).hexdigest(), index=-1),
            data="Seed data",
        )
        seed.chain = self
        seed.save()
    ...
    def add(self, data):
        ...
        block = Block.generate_next(self.last_block, data)
        block.chain = self
        return block
    ...
    def is_valid_chain(self, blocks=None):
        ...
        if len(blocks) == 1 and blocks[0].index != 0:
            ...
            return False
        if not all(
            pblock.index + 1 == block.index == required_index
            for pblock, block, required_index in zip(
                blocks[:-1], blocks[1:], range(1, len(blocks))
            )
        ):
            log.warning("Chain is not sequential")
            return False
        return all(
            block.is_valid_block(pblock)
...
class Peer(models.Model):
    ...
    def query_latest_block(self, chain_name):
        ...
        data = JsonApi.get(
            self.address, reverse("latest-block", kwargs={"chain_name": chain_name})
        )
    ...
    def query_chain(self, chain_name):
        ...
        data = JsonApi.get(self.address, reverse("chain", kwargs={"name": chain_name}))
    ...
    def fetch_longest_chain(self, chain_name):
        chain = max(
            (peer.query_chain(chain_name) for peer in self.discover_all_peers()),
            key=len,
        )
