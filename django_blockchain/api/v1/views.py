...
from chain.api.v0.serializers import (
    BlockSerializer,
    ChainSerializer,
    PeerSerializer,
...
class LatestBlockApiView(generics.GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        instance = (
            Block.objects.filter(chain__name=kwargs.get("chain_name"))
            .order_by("index")
            .last()
        )
        return Response(BlockSerializer(instance).data, status=status.HTTP_200_OK)
...
class PeerApiView(generics.ListAPIView, generics.CreateAPIView):
    permission_classes = (AllowAny,)
